...
const WishlistPage: React.FC = () => {
  ...
  const { addToCart } = useCart(); // Assuming there's a context for managing the cart

  const handleAddToCart = (item: WishlistItem) => {
    addToCart({ ...item, quantity: 1 }); // Add the item to the cart with quantity
  };
          ...
          <ul>
            {wishlist.map((item: WishlistItem) => (
              <li key={item._id} className="flex items-center justify-between border-b py-4">
                ...
                {/* Align Price, Trash, and Add to Cart Button */}
                <div className="flex items-center space-x-20 ml-auto">
                  ...
                  <button
                    ...
                    className="text-customGrey2 hover:text-customBlue mt-2"
                  >
                    <FaTrashCan size={20} />
                  </button>
                  ...
                  <button
                    ...
                    className="flex items-center bg-cBlue text-white rounded-md px-4 py-2 space-x-2"
                  >
